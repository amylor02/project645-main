...
class acLSTM(nn.Module):
    ...
    def init_hidden(self, batch):
        ...
        c0 = torch.autograd.Variable(
            torch.FloatTensor(np.zeros((batch, self.hidden_size))).cuda()
        )
        c1 = torch.autograd.Variable(
            torch.FloatTensor(np.zeros((batch, self.hidden_size))).cuda()
        )
        c2 = torch.autograd.Variable(
            torch.FloatTensor(np.zeros((batch, self.hidden_size))).cuda()
        )
        h0 = torch.autograd.Variable(
            torch.FloatTensor(np.zeros((batch, self.hidden_size))).cuda()
        )
        h1 = torch.autograd.Variable(
            torch.FloatTensor(np.zeros((batch, self.hidden_size))).cuda()
        )
        h2 = torch.autograd.Variable(
            torch.FloatTensor(np.zeros((batch, self.hidden_size))).cuda()
        )
    ...
    def forward(self, initial_seq, generate_frames_number):
        ...
        out_seq = torch.autograd.Variable(
            torch.FloatTensor(np.zeros((batch, 1))).cuda()
        )

        out_frame = torch.autograd.Variable(
            torch.FloatTensor(np.zeros((batch, self.out_frame_size))).cuda()
        )
...
def generate_seq(initial_seq_np, generate_frames_number, model, save_dance_folder):
    ...
    dif = (
        initial_seq_np[:, 1 : initial_seq_np.shape[1]]
        - initial_seq_np[:, 0 : initial_seq_np.shape[1] - 1]
    )
    initial_seq_dif_hip_x_z_np = initial_seq_np[
        :, 0 : initial_seq_np.shape[1] - 1
    ].copy()
    ...
    for b in range(batch):
        ...
        for frame in range(out_seq.shape[0]):
            ...
            out_seq[frame, Hip_index * 3 + 2] = (
                out_seq[frame, Hip_index * 3 + 2] + last_z
            )
            last_z = out_seq[frame, Hip_index * 3 + 2]

        output_path = os.path.join(save_dance_folder, "out" + "%02d" % b + ".bvh")
        read_bvh.write_traindata_to_bvh(output_path, out_seq)
...
def test(
    ...
    model.load_state_dict(torch.load(resolved_weight_path))
    # Move model to GPU
    model.cuda()
    ...
    for b in range(batch):
        ...
        start_id = random.randint(
            10, int(dance_len - initial_seq_len * speed - 10)
...
def main():
    ...
    parser.add_argument("--in_frame", type=int, default=171, help="Input channel")
    parser.add_argument("--out_frame", type=int, default=171, help="Output channels")
    parser.add_argument(
