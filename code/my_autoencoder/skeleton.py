...
class SkeletonLinear(nn.Module):
    def __init__(
        ...
        # global dummy_idx, dummy_idx_initial
        # dummy_idx_initial = self.param["extra_joint"]
        # dummy_idx = self.param["extra_joint"]
    ...
    def forward(self, input):
        expected_in_features = self.weight.shape[1]
        if input.shape[-1] < expected_in_features:
            input = torch.cat(
                [
                    input,
                    torch.zeros(
                        input.shape[0],
                        expected_in_features - input.shape[-1],
                        dtype=input.dtype,
                        device=input.device,
                    ),
                ],
        ...
        elif input.shape[-1] > expected_in_features:
            raise ValueError(
                f"SkeletonLinear expected at most {expected_in_features} input features, got {input.shape[-1]}"
            )
...
class SkeletonPool(nn.Module):
    def __init__(self, parents, channels_per_edge, device):
        ...
        # reset dummy index because create_pooling_list() will be called again from Encoder
        # global dummy_idx, dummy_idx_initial
        # dummy_idx = dummy_idx_initial
