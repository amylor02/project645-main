...
def eval_pos_error(gt_bvh, eval_bvh, device):
    # gt_rots, gt_pos, gt_parents, gt_offsets, _, _ = train_vq_vae.get_info_from_bvh(gt_bvh)
    ...
    # rots, pos, parents, offsets, _, _ = train_vq_vae.get_info_from_bvh(eval_bvh)
    ...
    # Modifying layer number needs this
    error = torch.norm(
        joint_poses[:,:min(joint_poses.shape[1],gt_joint_poses.shape[1]),...] 
        - gt_joint_poses[:, :min(joint_poses.shape[1],gt_joint_poses.shape[1]), ...], dim=-1   #<----------------------
    )
    ...
    # joint_poses - gt_joint_poses[:, : joint_poses.shape[1], ...], dim=-1
    # )
    # sparse_error = error[:, :, train.param["sparse_joints"][1:-1]]  # ignore root joint and dummy joint
    # return torch.mean(error).item(), torch.mean(sparse_error).item()
