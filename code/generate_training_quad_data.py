...
def generate_quad_traindata_from_bvh(src_bvh_folder, tar_traindata_folder):
	...
	for index, bvh_dance_name in enumerate(bvh_dances_names, start=1):
		...
		dance = read_bvh.get_quaternion_train_data(
			os.path.join(src_bvh_folder, bvh_dance_name),
			non_end_bones,
...
def generate_bvh_from_quad_traindata(src_train_folder, tar_bvh_folder):
	...
	for index, dance_name in enumerate(dances_names, start=1):
		...
		read_bvh.write_quaternion_traindata_to_bvh(
			os.path.join(tar_bvh_folder, dance_name + ".bvh"),
			dance,
			non_end_bones,
			translation_weight=weight_translation,
		)
...
def main():
	...
	parser.add_argument(
		"--output_npy_folder",
		type=str,
		default="train_data_quad/martial/",
		help="Target folder for quaternion NumPy files",
	)
	parser.add_argument(
		"--output_bvh_folder",
		type=str,
		default="reconstructed_bvh_data_quad/martial/",
	...
	generate_quad_traindata_from_bvh(args.src_bvh_folder, args.output_npy_folder)

	if args.skip_reconstruction:
		print("Skipping BVH reconstruction")
		return

	generate_bvh_from_quad_traindata(args.output_npy_folder, args.output_bvh_folder)
