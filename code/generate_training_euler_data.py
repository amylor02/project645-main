...
def generate_euler_traindata_from_bvh(src_bvh_folder, tar_traindata_folder):
    ...
    for index, bvh_dance_name in enumerate(bvh_dances_names, start=1):
        ...
        dance = read_bvh.get_euler_train_data(
...
def main():
    ...
    parser.add_argument(
        "--output_npy_folder",
        type=str,
        default="train_data_euler/martial/",
        help="Target folder for Euler NumPy files",
    )
    parser.add_argument(
        "--output_bvh_folder",
        type=str,
        default="reconstructed_bvh_data_euler/martial/",
    ...
    generate_euler_traindata_from_bvh(args.src_bvh_folder, args.output_npy_folder)

    if args.skip_reconstruction:
        print("Skipping BVH reconstruction")
        return

    generate_bvh_from_euler_traindata(args.output_npy_folder, args.output_bvh_folder)
